...
class ChatClient(wx.Frame):
    def __init__(self, client_name):
        ...
        # 创建两个按钮
        connect_btn = wx.Button(pl, size=(200, 20), label='连接')
        disconnect_btn = wx.Button(pl, size=(200, 20), label='断开')
        # 把两个按钮放到可伸缩的网格布局
        ...
        # 创建两个按钮
        reset_btn = wx.Button(pl, size=(200, 20), label='重置')
        send_btn = wx.Button(pl, size=(200, 20), label='发送')
        # 把两个按钮放到可伸缩的网格布局
        ...
        # 给重置按钮绑定事件
        self.Bind(wx.EVT_BUTTON, self.reset_msg, reset_btn)
        # 给断开链接绑定事件
        self.Bind(wx.EVT_BUTTON, self.disconnect, disconnect_btn)

    def disconnect(self, event):
        # 发送断开的消息
        data = f'tony-disconnect'
        self.client_socket.send(data.encode('utf-8'))
        # 改变连接状态
        self.isConnected = False

    def reset_msg(self, event):
...
if __name__ == '__main__':
    ...
    client.Show()  # 可以改成YsjClient('tony').Show();
    # 循环刷新显示
    app.MainLoop()
